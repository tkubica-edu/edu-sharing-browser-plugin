...
| `EVENT` / `REQ` to a nostr relay (AMB, kind 30142) | sidebar document (`NostrForwardService` → `util/nostr-relay.ts`) | nostr has no HTTP path at all: one WebSocket per exchange — `publishToRelay` closes with the relay's `OK`, `queryRelay` with its `EOSE`. Needs `wss:`/`ws:` in the extension's `connect-src`, which the scheme sources for `https:`/`http:` do not have to imply. Neither runs while the settings' *Nostr-Relay verwenden* is off, or while the deployment blacklists `nostr` (`APP_CONFIG.featureBlacklist`, see [FEATURES.md § An Nostr Relay weiterleiten](FEATURES.md#an-nostr-relay-weiterleiten)) |
