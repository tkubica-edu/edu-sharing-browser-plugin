...
`app-src/src/app/util/ai-schemas.ts`, the
...
`util/ai-prompts.ts`, one entry per step, and the shapes those answers are asked in stand
in `util/ai-schemas.ts`, one schema per step.
