...
export interface MetadataEditor {
  ...
  commit(): void;
  /**
   * The payload behind the committed values, for an editor that has one: the envelope it works against and every
   * value in the shape its field has, neither of which survives the `string[]` map {@link commit} emits. It is what
   * the metadata is re-read from after the save. Absent where the editor has nothing to add beyond the values.
   */
