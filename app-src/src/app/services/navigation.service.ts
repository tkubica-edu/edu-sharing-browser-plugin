...
export class NavigationService {
  ...
  /**
   * Whether the open section asks for a session of the user's own that this one is not. The section is
  ...
  /**
   * Back one step: to the section the user came from with the sub step they had open, and to the main
   * menu once the trail is used up. A step that no longer applies is skipped rather than opened, as is
   * one that must not be re-entered at all (see AppSection.oneWay).
   */
  back(): void {
    if (this.busy.busy()) return;
   ...
   * Nothing else opens itself — a node that was loaded surfaces as the *Inhalt erkannt* menu entry
  ...
  /**
   * Reopen a stored state: the step the user was on, with the steps behind it as the trail. Answers
   * whether anything was opened. Where the stored step cannot be re-entered here, the deepest step behind
   * it that still applies is opened instead, so the panel comes back as close as this page allows.
   */
  ...
  /**
   * Release a picked content when stepping back to a view that does not need one. The test is the target's
   * reachability in a world with no content at all: a section that is still reachable does not depend on
   * one. A detected content describes the open page and stays either way.
   */
