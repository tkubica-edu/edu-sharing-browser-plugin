...
| **Qualitätsprüfung** | nur der **Metadaten**-Tab | „Qualität"-Tab aus `mds_oeh` plus Gate: die Metadaten erst nach erfüllten Knock-out-Kriterien | Die **maschinelle Beurteilung läuft in beiden Kontexten** (`analyze()` → `judgeQuality()`); MetalookUp und ContentJudge lassen sich seit 15.09.2026 als zwei weitere `APP_CONFIG.featureBlacklist`-Einträge deployment-weit abschalten (siehe [STORE-RELEASE.md](../STORE-RELEASE.md) § B7), unabhängig vom WLO-Flag — die *Einstellungen*-Karte bleibt aber weiterhin hinter `wlo.enabled()`, ein Core-Deployment, das nur einen der beiden Judges will, erreicht dessen Schalter also noch nicht. Drei von elf Kriterien haben **kein Schema** (`data_privacy`, `copyright_law`, `relevancy_for_education`), und Barrierearmut hängt an genau einer AXE-Regel |
...
| **Einstellungen** | Repository-URL, Darstellung, **Entwickler-Optionen** (darin *WLO-Funktionen verwenden*), Nostr-Relay | zusätzlich KI/Chatbot, Sammlungsempfehlung, Qualitätsprüfung | Die Gruppe *Qualitätsprüfung* hängt weiterhin am WLO-Flag, obwohl **die Qualitätsjudges im Core laufen** → ein Core-Betreiber, der nur MetalookUp oder nur ContentJudge braucht, erreicht deren Schalter nicht (kann sie aber per `featureBlacklist` deployment-weit abschalten, statt sie ungesehen laufen zu lassen — siehe oben) |
...
| **Store-Fähigkeit** | `web-ext lint`: 0 Errors, ~195 Warnings, 0 Notices (alle aus den Vendor-Bundles); `data_collection_permissions` gesetzt, `clipboardRead` gestrichen, MetalookUp/ContentJudge per `featureBlacklist` aus — siehe [STORE-RELEASE.md](../STORE-RELEASE.md) | `host_permissions: https://*/*` und `connect-src https: wss: ws: data: blob:` → strenges Review erwartet; `dist/` ist **54 MB** (pdf.js 21 MB, TinyMCE 13 MB) |
...
2. **Die Settings-Gruppe Qualitätsprüfung aus dem WLO-Gate nehmen**, weil die Judges darunter im
   Core aktiv sind. Das unsichtbare-und-unabschaltbare Risiko selbst ist mit `metalookup`/
   `contentJudge` in `APP_CONFIG.featureBlacklist` (15.09.2026) entschärft; offen bleibt, dass ein
   Core-Deployment die Schalter für einen einzelnen, nicht blacklisteten Judge noch immer nicht
   erreicht.
